...
function get_router() {
    local interface="$1"
    local subnet_router_meta_path="computeMetadata/v1/instance/network-interfaces/1/gateway"
...
function post_status() {
    ...
    if "true" ; then
        ...
        $FWDIR/scripts/gcp.py POST "https://runtimeconfig.googleapis.com/v1beta1/projects/otc-core-network-prod-4aea/configs/gcp-vacgg03-config/variables"             --body '{
                "name": "projects/otc-core-network-prod-4aea/configs/gcp-vacgg03-config/variables/status/$status/$instance_id",
...
if ${installSecurityManagement} ; then
    if "$generatePassword" ; then
        managementAdminPassword="$(get-cloud-data.sh             computeMetadata/v1/instance/attributes/adminPasswordSourceMetadata)"
...
if "$generatePassword" ; then
    blink_password="$(get-cloud-data.sh         computeMetadata/v1/instance/attributes/adminPasswordSourceMetadata)"
else
    blink_password="$(dd if=/dev/urandom count=1         2>/dev/null | sha1sum | cut -c -28)"
fi
blink_conf="$blink_conf&admin_password_regular=$blink_password"

if [ "Gateway only" = "Cluster" ] || [ "Cluster" = "Cluster" ] || [ "AutoScale" = "Cluster" ]; then
